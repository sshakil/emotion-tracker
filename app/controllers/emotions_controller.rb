class EmotionsController < ApplicationController
  ...
  # GET /emotions or /emotions.json
  def index
    @emotions = Emotion.all
  end

  # GET /emotions/1 or /emotions/1.json
  def show
  end
  ...
  # POST /emotions or /emotions.json
  def create
  ...
  # PATCH/PUT /emotions/1 or /emotions/1.json
  def update
    respond_to do |format|
      if @emotion.update(emotion_params)
        format.html { redirect_to emotion_url(@emotion), notice: "Emotion was successfully updated." }
  ...
  # DELETE /emotions/1 or /emotions/1.json
  def destroy
    @emotion.destroy
